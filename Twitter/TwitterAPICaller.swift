...
class TwitterAPICaller: BDBOAuth1SessionManager {    
    static let client = TwitterAPICaller(baseURL: URL(string: "https://api.twitter.com"), consumerKey: "S1fflTppfvhfcjcMaMlgaxNMq", consumerSecret: "Nb5oAeijt6QBi6w72IReRHJPobUmKH8B1t5uOwkMd1XHsiJaft")
    var loginSuccess: (() -> ())?
    ...
    func retweet (tweetId:Int, success: @escaping() -> (), failure: @escaping (Error) -> ()){
        ...
        TwitterAPICaller.client?.post(url, parameters: ["id":tweetId],progress: nil, success:{(task: URLSessionDataTask, respnse: Any?) in
            success()
